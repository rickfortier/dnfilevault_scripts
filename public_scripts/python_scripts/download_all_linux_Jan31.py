"""
DNFileVault Downloader for Linux (Python)
------------------------------------------------------------------
This script downloads ALL files from your DNFileVault account:
1. All Purchases
2. All Groups

It is designed for Linux users and includes explanations for each part.
...
# On Linux, you can use paths like "~/dnfilevault-downloads" or "/home/user/downloads"
# We use expanduser to handle the '~' symbol correctly.
DEFAULT_PATH = os.path.expanduser("~/dnfilevault-downloads")
OUTPUT_FOLDER = os.environ.get("DNFV_OUT_DIR", DEFAULT_PATH)
...
def sanitize_filename(name):
    """
    Cleans up a filename so it is safe for Linux/Unix systems.
    """
    if not name:
        return "unnamed_file"
...
def main():
    print("--- DNFileVault Linux Downloader ---")
