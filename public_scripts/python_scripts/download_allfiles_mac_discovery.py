#!/usr/bin/env python3
"""
DNFileVault Downloader for macOS
------------------------------------------------------------------
...
BEFORE YOU RUN THIS:
1. macOS comes with Python 3 (Sonoma+). Check with: python3 --version
...
HOW TO RUN:
    python3 dnfilevault_downloader_mac.py

MAKE EXECUTABLE (optional):
    chmod +x dnfilevault_downloader_mac.py
    ./dnfilevault_downloader_mac.py

SCHEDULE WITH LAUNCHD (daily at 6am):
    See the included com.dnfilevault.downloader.plist example.
    Copy to ~/Library/LaunchAgents/ and load with:
        launchctl load ~/Library/LaunchAgents/com.dnfilevault.downloader.plist

ENVIRONMENT VARIABLES (recommended):
    export DNFV_EMAIL="your_email@example.com"
    export DNFV_PASSWORD="your_password"
    export DNFV_OUT_DIR="$HOME/Documents/DNFileVault"
"""
...
# Default: ~/Documents/DNFileVault (override with DNFV_OUT_DIR env variable)
OUTPUT_FOLDER = os.environ.get("DNFV_OUT_DIR", os.path.expanduser("~/Documents/DNFileVault"))
...
def sanitize_filename(name):
    """Clean up a filename for macOS filesystem."""
    if not name:
        return "unnamed_file"
    # macOS (APFS/HFS+) disallows : and / in filenames
...
def main():
    log("=" * 50)
    log("DNFileVault Downloader v2.0 (macOS)")
    log("=" * 50)
    ...
    if not base_url:
        ...
        send_notification("DNFileVault", "ERROR: All API servers unreachable")
        sys.exit(1)
    ...
    if not token:
        ...
        send_notification("DNFileVault", "Login failed - check credentials")
        sys.exit(1)
    ...
    for p in purchases:
        ...
        try:
            ...
            for f in files_to_download:
                before = os.path.exists(os.path.join(product_path, sanitize_filename(f.get("display_name", ""))))
                download_file(session, token, f, product_path, base_url)
                if not before and os.path.exists(os.path.join(product_path, sanitize_filename(f.get("display_name", "")))):
                    total_downloaded += 1
        except Exception as e:
    ...
    for g in groups:
        ...
        try:
            ...
            files_to_download = files[:DAYS_TO_CHECK] if DAYS_TO_CHECK is not None else files
            for f in files_to_download:
                before = os.path.exists(os.path.join(group_path, sanitize_filename(f.get("display_name", ""))))
                download_file(session, token, f, group_path, base_url)
                if not before and os.path.exists(os.path.join(group_path, sanitize_filename(f.get("display_name", "")))):
                    total_downloaded += 1
        except Exception as e:
    ...
    if total_downloaded > 0:
        send_notification("DNFileVault", f"Downloaded {total_downloaded} new file(s)")
    else:
        send_notification("DNFileVault", "Sync complete - all files up to date")
